...
metric.select <- function (Test,Reference,outlier.rem=T) {
  ...
  hg<-c("Richness","Simpson","Shannon","Intolerants Richness","Percent Intolerants","Percent EPT","EPT Richness","Ephem Richness",
        ...
        "Percent Non Chir Dip" , "O:E","CA1","CA2")

  hb<-c("Percent Dominance","Percent Oligochaeta","Percent Chironomidae","Percent Isopoda","Percent Amphipoda","FBI",
        "Trich as Hydropsychidae","Ephem as Baetidae","Percent Coleoptera","HBI", "Percent CIGH","Bray-Curtis","CA1","CA2")
  ...
  if ((!is.null(test.var)) & (length(test.var)<max(4,ceiling(1/5*nrow(data))))){
    ...
    if (("CA1" %in% colnames(raw.data)) & !("CA1" %in% test.var)) {
      test.var<-c(test.var,"CA1")
    }
    if (("HBI" %in% colnames(raw.data)) & !("HBI" %in% test.var)) {
      test.var<-c(test.var,"HBI")
    }
  ...
  if (is.null(test.var)){
    ...
    if (("CA1" %in% colnames(data)) & !("CA1" %in% test.var)) {
      test.var<-c(test.var,"CA1")
    }
    if (("HBI" %in% colnames(data)) & !("HBI" %in% test.var)) {
      test.var<-c(test.var,"HBI")
    }
