...
tsa.test<- function(Test, Reference, distance=NULL, outlier.rem=T, m.select=T) {
  ...
  if (m.select==T){
    data<-metric.select(Test=data.raw[nrow(data.raw),],Reference=data.raw[1:nRef,],outlier.rem=outlier.rem)$raw.data
  }
...
#' Z-Score Calculation
#'
#' This calculates z-scores centered on Reference Sites
#' @param Test Data frame of metric scores at the test site.
#' @param Reference Data frame of metric scores at the reference sites
#' @keywords z-score
#' @export
...
tsa.zscore<-function(Test,Reference) {
  ...
  tsa<-t(apply(tsa,1,function(x) (x-colMeans(Reference))/apply(Reference,2, sd)))
  tsa<-tsa[,complete.cases(t(tsa))]
  return(tsa)
}
